...
class Solution:
    def maxNumberOfFamilies(self, n: int, reservedSeats: List[List[int]]) -> int:
        ...
        for seats in reserved.values():
            ...
            if left.isdisjoint(seats):
                ans += 1
            if right.isdisjoint(seats):
                ans += 1

            # If neither left nor right family was possible,
            # the middle arrangement may still work.
            if (not left.isdisjoint(seats)
                    and not right.isdisjoint(seats)
                    and middle.isdisjoint(seats)):
                ans += 1
